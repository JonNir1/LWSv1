...
def _align_triggers_and_gaze(triggers, gaze) -> (pd.DataFrame, pd.DataFrame):
    ...
    triggers[cnfg.ACTION_STR] = triggers[cnfg.ACTION_STR].map(lambda act: SubjectActionCategoryEnum(act))
...
def _read_triggers(triggers_path: str) -> pd.DataFrame:
    ...
    # add `action` column
    triggers[cnfg.ACTION_STR] = SubjectActionCategoryEnum.NO_ACTION
    ...
    for idx, series in triggers.iterrows():
        ...
        if trg == cnfg.ExperimentTriggerEnum.SPACE_NO_ACT:
            ...
            triggers.loc[start_identify_idx, cnfg.ACTION_STR] = SubjectActionCategoryEnum.ATTEMPTED_MARK
            continue
        ...
        if trg in [
            ...
            if trg == cnfg.ExperimentTriggerEnum.CONFIRM_ACT:
                ...
                triggers.loc[start_identify_idx, cnfg.ACTION_STR] = SubjectActionCategoryEnum.MARK_AND_CONFIRM
            else:
                # subject rejects previously identified (non-target) item
                triggers.loc[start_identify_idx, cnfg.ACTION_STR] = SubjectActionCategoryEnum.MARK_AND_REJECT
            start_identify_idx = None
            continue
