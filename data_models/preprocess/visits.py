...
def convert_fixations_to_visits(
    ...
    for (trial, eye), subset in tqdm(
            all_fixations.groupby(by=[cnst.TRIAL_STR, cnst.EYE_STR]),
            desc="Extracting Visits", disable=True,
    ):
        subset = subset.sort_values(cnst.EVENT_STR, inplace=False).reset_index(drop=False)
        if subset.empty:
            continue
        ...
        for target_visit_col in visit_ids.columns:
            ...
            for vis_id in tgt_vis_ids.dropna().unique():
                visits.append(_extract_visit_features(
                    ...
                    visit_idx=vis_id,
                    trial=trial,
                    target=target_visit_col.replace(f"_{cnst.VISIT_STR}", ""),
                    eye=eye,
                ))
...
def _assign_visit_ids(
    ...
    trials = fixs_subset[cnst.TRIAL_STR].unique()
    if len(trials) != 1:
...
def _extract_visit_features(visit_fixs: pd.DataFrame, visit_idx: int, trial: int, target: str, eye: str,) -> dict:
    ...
    center_pixel_x = np.nansum(visit_fixs[cnst.X].values * durations.values) / np.nansum(durations)
    ...
    return {
        ...
        cnst.VISIT_STR: int(visit_idx),
        cnst.EVENT_STR: sorted(visit_fixs[cnst.EVENT_STR].values),
        ...
        cnst.X: center_pixel_x,
        cnst.Y: center_pixel_y,
        f"min_{cnst.DISTANCE_STR}_dva": visit_fixs[distance_col].min(),
